...
class TcpServer(
    ...
    fun stop() {
        ...
        DebugLogger.info("🔌 Tüm TCP port sunucuları kapatıldı")
    }
    ...
    fun writeData(data: ByteArray): Boolean {
        return try {
            ...
            DebugLogger.error("❌ writeData hatası (Video): ${e.message}")
            false
        }
    }
    ...
    private fun runVideoServer() {
        try {
            ...
            DebugLogger.success("✅ Video ServerSocket açıldı → PORT $PORT_VIDEO")
            ...
            while (running.get()) {
                try {
                    ...
                    DebugLogger.success("🔌 TFT Video/Projeksiyon bağlandı! → ${socket.inetAddress.hostAddress}:${socket.port}")
                    handleVideoClient(socket)
                } catch (e: SocketTimeoutException) {
                } catch (e: IOException) {
                    if (running.get()) {
                        DebugLogger.error("❌ Video accept hatası: ${e.message}")
                        Thread.sleep(1000)
                    }
                }
            }
        } catch (e: IOException) {
            DebugLogger.error("❌ Video ServerSocket bind hatası (Port $PORT_VIDEO): ${e.message}")
        }
    }
    ...
    private fun handleVideoClient(socket: Socket) {
        try {
            ...
            // 3. Heartbeat başlat (Port 15456 üzerinde 2s aralıklarla)
            ...
            DebugLogger.error("❌ Video client hatası: ${e.message}")
        } finally {
            connected.set(false)
            videoOutputStream = null
            videoHeartbeatThread?.interrupt()
            ...
            DebugLogger.warning("🔌 TFT Video bağlantısı koptu")
            onDisconnected()
        }
    }
        ...
        videoHeartbeatThread = Thread({
            ...
            try {
                while (running.get() && isClientConnected()) {
                    ...
                    os.write(packet)
                    os.flush()
                    DebugLogger.heartbeat("💓 Heartbeat gönderildi (Video - 15456)")
                }
            } catch (_: Exception) {}
    ...
    private fun sendVideoSizeHeader(os: OutputStream) {
        ...
        DebugLogger.data("📤 VideoSize header gönderildi (69 byte) → Port $PORT_VIDEO:")
        DebugLogger.data("   HEX[0..9]: $hexPreview...")
        DebugLogger.data("   Genişlik : $width px | Yükseklik: $height px")
    }

    private fun startTftVideoReader(inputStream: InputStream) {
        videoReaderThread = Thread({
            DebugLogger.info("👂 TFT→Telefon video reader başlatıldı")
            ...
            try {
                while (running.get() && connected.get()) {
                    ...
                    if (n > 0) {
                        ...
                        DebugLogger.data("📥 TFT→Tel (Video-15456): [$preview$more] toplam=$n byte")
                    }
                }
            } catch (_: Exception) {}
            DebugLogger.info("👂 TFT video reader durdu")
    ...
    private fun runControlServer() {
        try {
            ...
            DebugLogger.success("✅ Control ServerSocket açıldı → PORT $PORT_CONTROL")
            ...
            while (running.get()) {
                try {
                    ...
                    DebugLogger.success("🔌 TFT Control bağlandı! → ${socket.inetAddress.hostAddress}:${socket.port}")
            ...
            DebugLogger.error("❌ Control ServerSocket bind hatası (Port $PORT_CONTROL): ${e.message}")
        }
    }
    ...
    private fun handleControlClient(socket: Socket) {
        try {
            ...
            DebugLogger.info("📤 TFT'ye TUC GET sorgusu gönderiliyor...")
            ...
            controlReaderThread = Thread({
                DebugLogger.info("👂 TFT→Telefon control reader başlatıldı")
                val buf = ByteArray(4096)
                try {
                    while (running.get() && controlClientSocket?.isClosed == false) {
                        ...
                        if (n > 0) {
                            val preview = buf.take(minOf(n, 20)).joinToString(" ") { "%02X".format(it) }
                            val more = if (n > 20) " (+${n - 20}B)" else ""
                            DebugLogger.data("📥 TFT→Tel (Control-17818): [$preview$more] toplam=$n byte")
                            ...
                            if (!handshakeCompleted) {
                                ...
                                DebugLogger.success("✅ Control el sıkışması başarıyla tamamlandı!")
                            }
                        }
                    }
                } catch (_: Exception) {}
                DebugLogger.info("👂 TFT control reader durdu")
            ...
            controlReaderThread = null
            try { socket.close() } catch (_: Exception) {}
            DebugLogger.warning("🔌 TFT Control bağlantısı koptu")
        }
    }
    ...
    private fun sendBinaryControlHandshake(os: OutputStream) {
        ...
        os.flush()
        DebugLogger.info("📤 Control port binary el sıkışma paketleri gönderildi")
    }
    ...
    private fun runHeartbeatServer() {
        try {
            ...
            DebugLogger.success("✅ Heartbeat ServerSocket açıldı → PORT $PORT_HEARTBEAT")
            ...
            while (running.get()) {
                try {
                    ...
                    DebugLogger.success("🔌 TFT Dedicated Heartbeat bağlandı! → ${socket.inetAddress.hostAddress}:${socket.port}")
            ...
            DebugLogger.error("❌ Heartbeat ServerSocket bind hatası (Port $PORT_HEARTBEAT): ${e.message}")
        }
    }
    ...
    private fun startDedicatedHeartbeat(os: OutputStream) {
        ...
        dedicatedHeartbeatThread = Thread({
            val packet = byteArrayOf(0x02, 0x01, 0x00, 0x00, 0x00, 0x00)
            DebugLogger.info("💓 Dedicated Heartbeat gönderimi başladı (15457, 200ms aralık)")
            var count = 0L
            try {
                ...
                DebugLogger.warning("⚠️ Dedicated Heartbeat gönderim durdu: ${e.message}")
            }
            DebugLogger.info("💓 Dedicated Heartbeat gönderimi bitti")
